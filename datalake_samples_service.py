...
class DataLakeServiceSamples(object):
    ...
    connection_string = os.environ['DefaultEndpointsProtocol=https;AccountName=datalake427;AccountKey=RA/b7DA7TjderuRljVY+YcDljsfZZp3kX2ZTZA/rG0Dr2s8qzWUw3Kz2cD+AYUD5xdabb9p5VpXf+AStQmoDTw==;EndpointSuffix=core.windows.net']
    ...
    def upload(self):
        ...
        for filename in [f for f in os.listdir('data') if os.path.isfile(os.path.join('data',f))]:
            with open("data/"+filename,"rb") as source_file:
                data =source_file.read()

                file =DataLakeFileClient.from_connection_string(self.connection_string,file_system_name="data",file_path=filename)
